...
@section('page_name') المشتركين @endsection
@section('content')
    ...
    <div class="card mb-5 mb-xl-8 mt-10">
        ...
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">المشتركين في الموقع </span>
            </h3>
            <div class="card-toolbar">
                <a href="" class="btn btn-danger er fs-6 px-7 py-3" data-bs-toggle="modal" data-bs-target="#delete_all">حذف
                    الكل</a>
            </div>
        </div>
        <!--end::Header-->
                ...
                <table id="kt_datatable_example_5" class="table align-middle gs-0 gy-4">
                    ...
                    @foreach($offers as $offer)
                        <tr>
                            ...
                            <td>{{$offer->email}}</td>
                            <td>
                                {{ArabicDate($offer->created_at)['ar_day_not_current']}} -
                                {{date('j',strtotime($offer->created_at))}}
                                {{ArabicDate($offer->created_at)['month']}}
                                {{date('Y',strtotime($offer->created_at))}}
                            </td>
                            <td class="">
                                <button data-bs-toggle='modal' data-bs-target="#delete_offer"
                                        class="btn btn-icon btn-bg-light btn-danger btn-sm me-1" data-toggle="modal"
                                        data-id="{{$offer->id}}" data-email="{{$offer->email}}"
        ...
        <div class="modal-dialog modal-dialog-centered mw-650px">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-3 mx-xl-15 my-3">
                    <!--begin::Form-->
                    <form id="kt_modal_new_card_form" class="form" action="{{route('delete_all_offers')}}"
                          method="post">
                        <!--begin::Input group-->
                        {{csrf_field()}}
    ...
    <div class="modal fade" id="delete_offer" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-dialog modal-dialog-centered mw-650px">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-3 mx-xl-15 my-3">
                    <!--begin::Form-->
                    <form id="kt_modal_new_card_form" class="form" action="{{route('delete_one_offers')}}"
                          method="post">
                        {{csrf_field()}}

                        <div class="d-flex flex-column mb-7 fv-row fv-plugins-icon-container">
                            <!--begin::Label-->
                            <h4 class="pb-3">هل انت متاكد من حذف هذا المشترك ؟</h4>
                            <!--end::Label-->
                            <input type="hidden" class="form-control form-control-solid pt" name="id" id="id" value="">
                            <input type="text" class="form-control form-control-solid fs-2 pt" name="email" id="email" value="">
                        </div>
